...
const SignUp = () => {
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      toast({ title: "Google sign up failed", description: getAuthMessage(err), variant: "destructive" });
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-hero p-4">
      <div className="w-full max-w-md">
        ...
        <div className="rounded-3xl bg-card p-8 shadow-card border border-border/50">
          ...
          <p className="text-sm text-muted-foreground mb-6">Start your health journey today</p>

          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="name">Full name</Label>
              <Input id="name" type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Jane Doe" />
            </div>
            <div className="space-y-2">
